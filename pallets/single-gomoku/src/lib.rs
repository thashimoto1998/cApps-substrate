...
#[derive(Clone, Eq, PartialEq, PartialOrd, Ord, Encode, Decode, RuntimeDebug)]
struct GomokuState {
    board_state: Option<Vec<u8>>,
    stone_num: Option<u16>,
    stone_num_onchain: Option<u16>,
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn update_by_state(
            ...
            let board_state = state_proof.app_state.board_state;
            ensure!(
                board_state.len() == 227,
            ...
            let count = 0;
            if board_state[0] != 0 {
                gomoku_info = Self::win_game(board_state[0], app_id)?;
            } else {
                // advance to board_state[2];
                let mut board_state_iter = board_state.iter();
                for i in 0..3 {
                    board_state_iter.next();
                }
                // load other states only if winner is not specified
                let count = board_state_iter.filter(|&x| *x != 0).count() as u8;
        ...
        fn update_by_action(
            ...
            {
                new_gomoku_info_1 = Self::win_game(turn, app_id)?;
        ...
        fn finalize_on_action_timeout(
            ...
            let board_state = gomoku_info.gomoku_state.board_state.unwrap();
            if board_state[1] == 1 {
                let new_gomoku_info = Self::win_game(2, app_id)?;
                SingleGomokuInfoMap::<T>::mutate(app_id, |info| *info = Some(new_gomoku_info.clone()));
            } else if board_state[1] == 2 {
                let new_gomoku_info = Self::win_game(1, app_id)?;
                SingleGomokuInfoMap::<T>::mutate(app_id, |info| *info = Some(new_gomoku_info.clone()));
            } else {
                return Ok(());
            }
...
impl<T: Trait> Module<T> {
    ...
    fn win_game(
        ...
        let gomoku_info = Self::gomoku_info(app_id).unwrap();
        let gomoku_state = gomoku_info.gomoku_state;
        let mut new_board_state = gomoku_state.board_state.unwrap();
        // set winner
